...
#define INITIAL_ADV_TIMEOUT            (1.5*1000*1000/CFG_TS_TICK_VAL) /**< 1.5s */
#define SCAN_TIMEOUT                   (2*1000*1000/CFG_TS_TICK_VAL)  // 2s
...
static const char a_LocalName[] = {AD_TYPE_COMPLETE_LOCAL_NAME ,'N','3'};
uint8_t a_ManufData[14] = {sizeof(a_ManufData)-1,
                          AD_TYPE_MANUFACTURER_SPECIFIC_DATA,
                          0x01,                               /*SKD version */
                          CFG_DEV_ID_P2P_SERVER3,             /* STM32WB - P2P Server 1*/
...
void APP_BLE_Init(void)
{
  ...
  */
  ...
  while (__HAL_TIM_GET_COUNTER(&htim2) < 14900){
                  ...
                  switch (adtype)
                  {
                    ...
                    case AD_TYPE_MANUFACTURER_SPECIFIC_DATA: /* Manufacturer Specific */
                      ...
                      { /* ST VERSION ID 01 */
                        ...
                        {   /* Demo ID */
                          case CFG_DEV_ID_P2P_SERVER2: /* Device 1 */

                            CurrentTime.time = __HAL_TIM_GET_COUNTER(&htim2);
                            if (ReceivedTime.time >= PreviousTime.time)
                            {
                              myTime = CurrentTime.time + 0.1*(ReceivedTime.time - PreviousTime.time);
                        	  __HAL_TIM_SET_COUNTER(&htim2, myTime);
                            }
                            else if (ReceivedTime.time < PreviousTime.time)
                            {
                            	myTime = CurrentTime.time - 0.1*(PreviousTime.time - ReceivedTime.time);
                            	__HAL_TIM_SET_COUNTER(&htim2, myTime);
                            ...
                            APP_DBG_MSG("Current Time in ms is: %lu \n\r", CurrentTime.time);
                            APP_DBG_MSG("Received Time in ms is: %lu \n\r", ReceivedTime.time);
                            APP_DBG_MSG("My adjusted Time in ms is: %lu \n\r", myTime);
                    ...
                    case AD_TYPE_SERVICE_DATA: /* service data 16 bits */
                      /* USER CODE BEGIN AD_TYPE_SERVICE_DATA */
                    	ReceivedTime.bytes[0]= adv_report_data[k + 4];
                    	ReceivedTime.bytes[1]= adv_report_data[k + 5];
                    	ReceivedTime.bytes[2]= adv_report_data[k + 6];
...
static void Adv_Request(void)
{
  ...
  service_data[2] = 0x1A;
  service_data[3] = 0X18;
  service_data[4] = CurrentTime.bytes[0];
  service_data[5] = CurrentTime.bytes[1];
  service_data[6] = CurrentTime.bytes[2];
